...
def memory_sampling_balanced(opts, curr_step, prev_model):
    ...
    train_dst = dataset(opts=opts, image_set='train', transform=transform, cil_step=curr_step-1)
    ...
    memory_json = f'./data/{opts.dataset}/memory.json'
    
    if curr_step > 1:
        with open(memory_json, "r") as json_file:
            memory_list = json.load(json_file)

        memory_candidates = memory_list[f"step_{curr_step-1}"]["memory_candidates"]
    else:
        memory_list = {}
        memory_candidates = []
    ...
    memory_list[f"step_{curr_step}"] = {"memory_candidates": sampled_memory_list, 
